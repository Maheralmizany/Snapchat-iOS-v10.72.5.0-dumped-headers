//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "SCOverlayFilterView.h"

@class SCExperimentManager, SCLens, SCUcoInfoView;

@interface SCPreviewFeatureSnap3DLensFilterView : SCOverlayFilterView
{
    SCUcoInfoView *_lensView;
    SCExperimentManager *_experimentManager;
    _Bool _isTransitioning;
    SCLens *_lens;
}

+ (id)filterNameForLens:(id)arg1;
@property(nonatomic) _Bool isTransitioning; // @synthesize isTransitioning=_isTransitioning;
@property(readonly, nonatomic) SCLens *lens; // @synthesize lens=_lens;
- (void).cxx_destruct;
@property(readonly, nonatomic) _Bool isLensInfoButtonShouldBeShown;
- (void)hideOverlay;
- (void)startViewing;
- (_Bool)shouldRespondToTap:(id)arg1;
- (_Bool)hasImage;
- (id)displayName;
- (id)initWithFrame:(struct CGRect)arg1 config:(id)arg2 userSession:(id)arg3;

@end

