//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

#import "NSCopying.h"

@class NSString;

@interface SCComposerAvatarUserInfo : NSObject <NSCopying>
{
    _Bool _hasBirthdayToday;
    NSString *_userId;
    NSString *_username;
    NSString *_bitmojiAvatarId;
    NSString *_bitmojiSelfieId;
}

@property(readonly, nonatomic) _Bool hasBirthdayToday; // @synthesize hasBirthdayToday=_hasBirthdayToday;
@property(readonly, copy, nonatomic) NSString *bitmojiSelfieId; // @synthesize bitmojiSelfieId=_bitmojiSelfieId;
@property(readonly, copy, nonatomic) NSString *bitmojiAvatarId; // @synthesize bitmojiAvatarId=_bitmojiAvatarId;
@property(readonly, copy, nonatomic) NSString *username; // @synthesize username=_username;
@property(readonly, copy, nonatomic) NSString *userId; // @synthesize userId=_userId;
- (void).cxx_destruct;
- (_Bool)isEqual:(id)arg1;
- (unsigned long long)hash;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (id)initWithUserId:(id)arg1 username:(id)arg2 bitmojiAvatarId:(id)arg3 bitmojiSelfieId:(id)arg4 hasBirthdayToday:(_Bool)arg5;

@end

